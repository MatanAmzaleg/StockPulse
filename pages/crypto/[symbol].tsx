...
export default function CryptoDetails({
  ...
  const handleTransaction = async (action: string, amount: number) => {
    try {
      const userEmail = getCookie("loggedInUser");
      const res = await axios.get(`/api/user/getcoins?username=${userEmail}`);
      ...
      switch (action) {
        case "buy":
          if (amount > userCoins) return console.log("need more cash to preform action");
          await axios.post(`/api/user/transactions/add`, {
            transaction,
            email: userEmail,
          });
          await axios.post(`/api/user/crypto/buy`, {
            email: userEmail,
            crypto,
          });
          break;
    ...
    <section className="crypto-details">
      ...
      <div className="main-grid">
        <div className="card small-detaills">
          ...
          <h3 className="price">{formattedPrice(alpacaCrypto!.bp)}</h3>
          <div className="">
            <div className="percentage-container">
              ...
              <p
                className={
                  Number(calculateChange(data.o, alpacaCrypto?.ap)) < 0
                    ? "descending"
                    : "scending"
                }
              >
        ...
        <div className="card open-close">
          ...
          <table>
            ...
            <tr>
              ...
              <td className="table-date">{dateTimeFormat(new Date(oc.ts))}</td>
        ...
        <div className="card actions flex column">
          ...
          <div className="flex">
            <button
              className="buy"
              onClick={() => handleTransaction("buy", inputValue)}
            >
              Buy
            </button>
            <button
              className="sell"
              onClick={() => handleTransaction("sell", inputValue)}
            >
              Sell
            </button>
          </div>
